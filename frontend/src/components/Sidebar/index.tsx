...
const Sidebar: React.FC = () => {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      toast.success(t('meeting_deleted'), {
        description: t('meeting_delete_description')
      });
      ...
      toast.error(t('meeting_delete_failed'), {
  ...
  const handleEditConfirm = async () => {
    ...
    if (!newTitle) {
      toast.error(t('meeting_title_empty'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('meeting_title_updated'));
      ...
      <TooltipProvider>
        <div className="flex flex-col items-center space-y-4 mt-4">
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{isRecording ? t('recording_in_progress') : t('nav_start_recording')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          {item.type === 'folder' ? (
            <>
              ...
              {searchQuery && item.id === 'meetings' && isSearching && (
                <span className="ml-2 text-xs text-blue-500 animate-pulse">{t('sidebar_searching')}</span>
              )}
            </>
          ) : (
            <div className="flex flex-col w-full">
                ...
                {isMeetingItem && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      aria-label={t('meeting_edit_title')}
                    >
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                  <span className="font-medium text-yellow-600">{t('sidebar_match_label')}</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
      ...
      <div
        className={`h-screen bg-white border-r shadow-sm flex flex-col transition-all duration-300 ${isCollapsed ? 'w-10' : 'w-56'
          }`}
      >
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder={t('sidebar_search_content_placeholder')} value={searchQuery}
            ...
            >
              {isRecording ? (
                <>
                  ...
                  <span>{t('recording_in_progress')}</span>
                </>
              ) : (
                <>
                  <Mic className="w-4 h-4 mr-2" />
                  <span>{t('recording_start')}</span>
                </>
              )}
            </button>
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>{t('meeting_edit_title')}</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">{t('meeting_edit_title')}</h3>
            <div className="space-y-4">
              <div>
                <label htmlFor="meeting-title" className="block text-sm font-medium text-gray-700 mb-2">
                  {t('meeting_title_label')}
                </label>
                <input
                  id="meeting-title"
                  type="text"
